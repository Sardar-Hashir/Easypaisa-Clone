...
DATA SEGMENT
    ;Account number
    ACC_PROMPT DB 0AH, " Enter your Phone number: $"
    ACC_WRONG DB 0AH, 0AH, 0DH, " phone number does not exist.$"
    ACC_NUM DB "03457045011$"
    ...
    ;Menu options
    WEL_MSG DB 0AH, 0AH, 0AH, 0DH, "**********************  Welcome to your easypaisa account  *********************$"
    BAL_MENU DB 0AH, 0AH, 0DH, " 1. Check your balance$"
    WITH_MENU DB 0AH, 0DH, " 2. transfer money$"
    ...
    WITH_PHONE DB 0AH, 0AH, 0DH, " Enter Phone number:  $"  
    WITH_A DW 0H 
    WITH_PROMPT DB 0AH, 0AH, 0DH, " Enter amount to transfer: Rs. $"
    WITH_AMT DW 0H 
...
CODE SEGMENT
    ...
    ; Display a string using DOS interrupt 21H
    MOV AH, 09H      
    LEA DX, WEL_MSG  
    INT 21H         
    ...
    CMP AL, 30H            ; Compare AL with '0' (ASCII 30H)
    JL INP_ERROR           ; Jump to error if AL < '0'
    CMP AL, 39H            ; Compare AL with '9' (ASCII 39H)
    JG INP_ERROR           ; Jump to error if AL > '9'

    INC AX                 ; Increment the valid digit count in AX
